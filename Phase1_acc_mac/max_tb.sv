...
logic                          result_valid;
logic signed [D_W-1:0]        result;
logic         [$clog2(M2)-1:0] cntr;

logic  signed [D_W-1:0] in_memory  [MAT-1:0];
logic  signed [D_W-1:0] out_memory [M1-1:0];
...
initial begin
    ...
    $readmemh("max_out.mem", out_memory);
end
...
always @(posedge clk) begin
    if (rst[0]) begin
        ...
        initialize   <= 0;
        result_valid <= 0;
        $display("# Time=%0d, in_data=%0d, cntr=%0d, init=%0d, result_valid=%0d, result=%0d", $time, in_data, cntr, initialize, result_valid, result);
        if (in_addr == 0) begin
            initialize <= 1;
        end
        if (in_v) begin
