...
declare -A addr_impl_sim_pp
addr_impl_sim_pp=(MEM_B 0 MEM_D 0)
for dut in MEM_B MEM_D; do
    make addr-impl N=8 DUT=$dut BLOCKS=8 MEM_DEPTH=49152
    make addr-xsim M1=32 M2=768 M3=64 N1=8 N2=8 DUT=$dut MODE=impl BLOCKS=8 MEM_DEPTH=49152 | grep "Error\|PASSED" > addr_impl_pp.txt 2>&1
    addr_impl_pp_pass=`grep "PASSED" addr_impl_pp.txt | wc -l`
    if [[ $addr_impl_pp_pass -eq 1 ]]; then addr_impl_sim_pp[$dut]=5 else addr_impl_sim_pp[$dut]=0; fi
done
...
if [ -e "mem_write_B_impl_util_pp.txt" ]; then
    mem_write_B_impl_gold_lut_pp=`cat mem_write_B_impl_util_pp.golden | grep "Slice LUTs" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g`
    mem_write_B_impl_gold_ff_pp=`cat mem_write_B_impl_util_pp.golden | grep "Slice Registers" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g`

    mem_write_B_impl_lut_pp=`cat mem_write_B_impl_util_pp.txt | grep "Slice LUTs" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g`
    mem_write_B_impl_ff_pp=`cat mem_write_B_impl_util_pp.txt | grep "Slice Registers" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g | head -1`
    mem_write_B_impl_dsp_pp=`cat mem_write_B_impl_util_pp.txt | grep "DSPs" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g`

    mem_write_B_impl_diff_lut_pp=`expr 100 \* \( $mem_write_B_impl_lut_pp - $mem_write_B_impl_gold_lut_pp \) \/ $mem_write_B_impl_gold_lut_pp`
    mem_write_B_impl_diff_ff_pp=`expr 100 \* \( $mem_write_B_impl_ff_pp - $mem_write_B_impl_gold_ff_pp \) \/ $mem_write_B_impl_gold_ff_pp`

    if [[ $mem_write_B_impl_lut_pp -eq 0 || $mem_write_B_impl_ff_pp -eq 0 || $mem_write_B_impl_diff_lut_pp -gt 20 || $mem_write_B_impl_diff_ff_pp -gt 20 || $mem_write_B_impl_dsp_pp -gt 0 ]]; then mem_write_B_impl_util_pp=0; else mem_write_B_impl_util_pp=2; fi
else
    mem_write_B_impl_util_pp=0
fi
if [ -e "mem_read_D_impl_util_pp.txt" ]; then
    mem_read_D_impl_gold_lut_pp=`cat mem_read_D_impl_util_pp.golden | grep "Slice LUTs" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g`
    mem_read_D_impl_gold_ff_pp=`cat mem_read_D_impl_util_pp.golden | grep "Slice Registers" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g`

    mem_read_D_impl_lut_pp=`cat mem_read_D_impl_util_pp.txt | grep "Slice LUTs" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g`
    mem_read_D_impl_ff_pp=`cat mem_read_D_impl_util_pp.txt | grep "Slice Registers" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g | head -1`
    mem_read_D_impl_dsp_pp=`cat mem_read_D_impl_util_pp.txt | grep "DSPs" | head -n 1 | cut -d"|" -f 3 | sed s/\ //g`

    mem_read_D_impl_diff_lut_pp=`expr 100 \* \( $mem_read_D_impl_lut_pp - $mem_read_D_impl_gold_lut_pp \) \/ $mem_read_D_impl_gold_lut_pp`
    mem_read_D_impl_diff_ff_pp=`expr 100 \* \( $mem_read_D_impl_ff_pp - $mem_read_D_impl_gold_ff_pp \) \/ $mem_read_D_impl_gold_ff_pp`

    if [[ $mem_read_D_impl_lut_pp -eq 0 || $mem_read_D_impl_ff_pp -eq 0 || $mem_read_D_impl_diff_lut_pp -gt 20 || $mem_read_D_impl_diff_ff_pp -gt 20 || $mem_read_D_impl_dsp_pp -gt 0 ]]; then mem_read_D_impl_util_pp=0; else mem_read_D_impl_util_pp=2; fi
else
    mem_read_D_impl_util_pp=0
fi
...
if [[ -e "mem_write_B_impl_timing_pp.txt" && $addr_impl_sim_pp[MEM_B] -ne 0 ]]; then
    mem_write_B_impl_timing_met_pp=`grep "MET" mem_write_B_impl_timing_pp.txt | wc -l`
    mem_write_B_impl_slack_pp=`cat mem_write_B_impl_timing_pp.txt | grep Slack | cut -d" " -f11 | cut -d"n" -f1`
    if [[ $mem_write_B_impl_timing_met_pp -eq 1 || $mem_write_B_impl_slack_pp -gt -0.3 ]]; then mem_write_B_impl_timing_pp=2 else mem_write_B_impl_timing_pp=0; fi
else
    mem_write_B_impl_timing_pp=0
fi
if [[ -e "mem_read_D_impl_timing_pp.txt" && $addr_impl_sim_pp[MEM_D] -ne 0 ]]; then
    mem_read_D_impl_timing_met_pp=`grep "MET" mem_read_D_impl_timing_pp.txt | wc -l`
    mem_read_D_impl_slack_pp=`cat mem_read_D_impl_timing_pp.txt | grep Slack | cut -d" " -f11 | cut -d"n" -f1`
    if [[ $mem_read_D_impl_timing_met_pp -eq 1 || $mem_read_D_impl_slack_pp -gt -0.3 ]]; then mem_read_D_impl_timing_pp=2 else mem_read_D_impl_timing_pp=0; fi
else
    mem_read_D_impl_timing_pp=0
fi
...
echo "addr_impl_sim_pp[MEM_B]: ${addr_impl_sim_pp[MEM_B]}" >> grade.csv
echo "addr_impl_sim_pp[MEM_D]: ${addr_impl_sim_pp[MEM_D]}" >> grade.csv
...
echo "mem_write_B_impl_util_pp: $mem_write_B_impl_util_pp" >> grade.csv
echo "mem_read_D_impl_util_pp: $mem_read_D_impl_util_pp" >> grade.csv
...
echo "mem_write_B_impl_timing_pp: $mem_write_B_impl_timing_pp" >> grade.csv
echo "mem_read_D_impl_timing_pp: $mem_read_D_impl_timing_pp" >> grade.csv
