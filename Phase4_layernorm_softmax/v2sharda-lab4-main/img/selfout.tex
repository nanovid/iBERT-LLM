...
\begin{document}
    ...
    \node [draw, circle, fill=lightRed, minimum size=\multSize, right=\rectWidth*0.5pt of multAtt] (addAtt) {\textbf{+}};
    ...
    \metarect{A_tilde}{$\tilde{A}$}{(32,768)}{8b}{24KB}{lightYellow}{right=\rectWidth*0.75pt of ln1.center}    

    \draw [-latex] (concat.south) -- (C.west);
    \draw [-latex] (C.east) -- (multAtt.west);
    \draw [-latex] (weight_att.south) -- (multAtt.north);
    \draw [-latex] (multAtt.east) -- (addAtt.west);
    \draw [-latex] (addAtt.east) -- (A.west);
    \draw [-latex] (A.east) -- (ln1.north);
    \draw [-latex] (ln1.south) -- (A_tilde.west);


    \draw [-latex] (layer_input.north) -- ++(0pt,\rectHeight*3.75pt) -| (addAtt.north);
