...
(
    ...
    output logic [ADDR_W_A-1:0]     rd_addr_A,
    output logic [ADDR_W_B-1:0]     rd_addr_B,
    output logic                     done_read_control,
    output logic [N2-1:0]           init [N1-1:0]
);

logic [MATRIXSIZE_W-1:0] pixel_cntr_A;
logic [MATRIXSIZE_W-1:0] slice_cntr_A;
logic [MATRIXSIZE_W-1:0] pixel_cntr_B;
logic [MATRIXSIZE_W-1:0] slice_cntr_B;

assign rd_addr_A = (slice_cntr_A * M2 + pixel_cntr_A);
assign rd_addr_B = (pixel_cntr_B * M2 + slice_cntr_B);

logic [MATRIXSIZE_W-1:0] e_patch_cntr       = 1;
logic             enable_row_count_A = 0;
logic [N2-3:0]    done_read_control_count_down;
...
logic [N1+N2-1:0] shift = 0;
logic [MATRIXSIZE_W-1:0] i_patch_cntr = 0;
