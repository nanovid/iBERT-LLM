...
always @(posedge clk) begin
    if (rst) begin
        ...
        if (s_axis_s2mm_tlast_A_r && s_axis_s2mm_tvalid_A_r) begin
            s_axis_s2mm_tready_A <= 0;
        end else if (done_multiply_sync[1] && ~tlast_A_flag) begin
            s_axis_s2mm_tready_A <= 1;
        end

        if (s_axis_s2mm_tlast_B_r && s_axis_s2mm_tvalid_B_r) begin
...
always @(posedge clk) begin
    if (rst) begin
        ...
        if (s_axis_s2mm_tlast_A_r && s_axis_s2mm_tvalid_A_r) begin
            tlast_A_flag <= 1;
        end else if (~reg_banked_valid_A[N1-1] && tlast_A_flag) begin
            write_done_A <= 1;
        end

        if (s_axis_s2mm_tlast_B_r && s_axis_s2mm_tvalid_B_r) begin
            tlast_B_flag <= 1;
        end else if (~reg_banked_valid_B[N2-1] && tlast_B_flag) begin
            write_done_B <= 1;
        end
    	...
    	if (start_multiply_sync[1]) begin
            ...
            tlast_A_flag <= 0;
            tlast_B_flag <= 0;
        end else if (write_done_A && write_done_B)
            write_done <= 1;
    end
end
/*
always @(posedge clk) begin
    if (rst) begin
        write_done_sync_wait <= 0;
    end else begin
        if (write_done) begin
            write_done_sync_wait <= 1;
        end else if (start_multiply_sync[1]) begin
            write_done_sync_wait <= 0;
        end
    end
end
...
always @(posedge fclk) begin
    write_done_sync <= {write_done_sync[0], write_done_sync_wait};
end
*/
always @(posedge fclk) begin
