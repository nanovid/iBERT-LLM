...
(
    ...
    (* keep = "true" *) input  wire                    done_rd,
    (* keep = "true" *) input  wire                    done_wr,
    (* keep = "true" *) input  wire                    done_wr_early,   //for stalling axi
    (* keep = "true" *) output reg   [1:0]             blk_0,           //{rd_en,wr_en}
    (* keep = "true" *) output reg   [1:0]             blk_1,           //{rd_en,wr_en}
    (* keep = "true" *) output reg                     stall_axi_b
);
...
initial begin
    $monitor("Time: %0t, State: %s, rd_en_0: %0b, wr_en_0: %0b, rd_en_1: %0b, wr_en_1: %0b", $time, curr_state.name(), blk_0[1], blk_0[0], blk_1[1], blk_1[0]);
end
